...
The teacher will give each student a problem starting with the student number 0, then the student number 1,
   and so on until the teacher reaches the student number n - 1. After that, the teacher will restart the process,
...
def chalkReplacer(chalk, k):
    ...
    :rtype: int
    """
    if sum(chalk) == k or k % sum(chalk) == 0:
        return 0
    k = k % sum(chalk)
    while k > sum(chalk):
        k -= sum(chalk)
...
Chalks = list(map(int, input().rstrip().split()))
K = int(input())
print(chalkReplacer(Chalks, K))
